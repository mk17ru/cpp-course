...
TEST(ExecutedOrderTest, decode_short_token_executed_order)
{
    ...
    // Algorithmic trade
    message.push_back('H');
    // Removed, Internalized, Self traded
    message.push_back(0b10110000);
    ASSERT_EQ(44, message.size());

    ExecutionDetails details = decode_executed_order(message);

    ASSERT_EQ(std::string("1234"), details.cl_ord_id);
    ASSERT_EQ(268559727, details.match_number);
    ASSERT_EQ(100000, details.filled_volume);
    ASSERT_DOUBLE_EQ(14.746, details.price);
    ASSERT_EQ(std::string("BofA"), details.counterpart);
    ASSERT_EQ(LiquidityIndicator::None, details.liquidity_indicator);
    ASSERT_TRUE(details.internalized);
    ASSERT_TRUE(details.self_trade);
}
...
TEST(ExecutedOrderTest, decode_thin_price_executed_order)
{
    ...
    ASSERT_EQ(74847, details.match_number);
    ASSERT_EQ(450000, details.filled_volume);
    ASSERT_DOUBLE_EQ(1.0001, details.price);
    ASSERT_EQ(std::string("SGen"), details.counterpart);
    ASSERT_EQ(LiquidityIndicator::Added, details.liquidity_indicator);
    ASSERT_FALSE(details.internalized);
    ASSERT_TRUE(details.self_trade);
